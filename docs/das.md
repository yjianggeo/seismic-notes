...
!!! tip "没有参考台站？从数据本身检测耦合状态"
    上述方法需要在同址布设点式地震仪。对于海底或长距离光缆这往往不可行——此时可以利用**邻道谱指纹的空间不规则性**直接从 DAS 数据反推耦合状态（埋设/暴露/悬空），例如 Poincaré 谱相干方法与埋设指数（BI），详见 [DAS 光缆耦合状态的检测与评估](cable-coupling.md)。
